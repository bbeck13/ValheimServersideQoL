...
public sealed class Config(ConfigFile cfg, Logger logger) : ConfigBase<Config>(cfg, logger)
{
  ...
  public ConfigEntry<bool> SpawnOneStarViles { get; } = BindEx(cfg, false, """
    True to let 1-star Viles spawn naturally.
    In vanilla, they can only be spawned with the developer console.
    """);

  public ConfigEntry<float> OneStarSpawnChanceMultiplier { get; } = BindEx(cfg, 1f, $"""
  ...
  public ConfigEntry<bool> SpawnTwoStarViles { get; } = BindEx(cfg, false, """
    True to let 2-star Viles spawn naturally.
    In vanilla, they can only be spawned with the developer console.
    """);

  public ConfigEntry<float> TwoStarSpawnChanceMultiplier { get; } = BindEx(cfg, 1f, $"""
